...
with open(provider_mappings_path, 'r') as f:
    PROVIDER_MAPPINGS = yaml.safe_load(f)

def get_stdlib_modules() -> Set[str]:
    """Get set of Python standard library module names."""
    return {
        'datetime', 'time', 'json', 'os', 'sys', 'logging', 're', 'pathlib',
        'collections', 'itertools', 'functools', 'typing', 'abc', 'copy',
        'contextlib', 'warnings', 'tempfile', 'shutil', 'random', 'uuid',
        'hashlib', 'base64', 'urllib', 'http', 'email', 'xml', 'html',
        'csv', 'sqlite3', 'pickle', 'gzip', 'zipfile', 'tarfile', 'configparser'
    }
...
def get_database_access_modules() -> Set[str]:
    ...
    return {
        ...
        'sqlalchemy.ext', 'sqlalchemy.dialects', 'sqlalchemy.pool',
        'sqlalchemy.func', 'func', 'orm', 'Session',
        'sqlalchemy.orm.session', 'sqlalchemy.orm.query',
        'sqlalchemy.sql.expression', 'sqlalchemy.engine.create',
        'pymysql', 'psycopg2', 'mysql.connector',
        'airflow.utils.db', 'airflow.utils.db_cleanup', 'airflow.utils.session',
        'airflow.utils.sqlalchemy', 'provide_session', 'NEW_SESSION'
    }
    ...
    def _process_import(self, name: str, node: ast.AST):
        ...
        if name in self.db_modules:
            self.issues.append({
                ...
                'message': f'Direct database access detected: {name}',
                'line': node.lineno
            })
...
class TopLevelCodeAnalyzer(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        self.sqlalchemy_patterns = {
            'methods': {'execute', 'query', 'commit', 'rollback', 'close', 'connect',
                       'create_engine', 'sessionmaker', 'scoped_session', 'min', 'max',
                       'scalar', 'all', 'first', 'one', 'count', 'select_from'},
            'classes': {'Engine', 'Connection', 'Session', 'Query', 'MetaData', 'Table',
                       'func', 'orm', 'session'},
            'modules': {'sqlalchemy', 'orm', 'engine', 'session', 'query', 'func'},
            'db_functions': {'run_cleanup', 'purge_table', 'resetdb',
                           'initdb', 'upgradedb', 'check_migrations',
                           'reflect_tables', 'provide_session', 'NEW_SESSION'}
        }
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            ...
            if func_name in self.sqlalchemy_patterns['methods']:
                self.issues['db_operations'].append({
                    'type': 'db_operation',
                    'message': f'Database operation at top level: {func_name}',
                    'line': node.lineno
                })
            ...
            if isinstance(node.func.value, ast.Name):
                if node.func.value.id in self.sqlalchemy_patterns['modules']:
                    self.issues['db_operations'].append({
                        'type': 'db_operation',
                        'message': f'Database operation at top level: {node.func.value.id}.{node.func.attr}',
                        'line': node.lineno
                    })
...
def find_provider_for_package(package: str) -> Optional[str]:
    ...
    for package in imports.get('third_party', set()):
        ...
        if provider:
            issues.append({
                ...
                'message': f'Consider using {provider} instead of {package}',
                'package': package,
                'provider': provider
            })
    return issues
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.BinOp) and isinstance(node.op, (ast.RShift, ast.LShift)):
            deps.append({
                'from': node.left,
                'to': node.right,
                'type': '>>' if isinstance(node.op, ast.RShift) else '<<'
            })
