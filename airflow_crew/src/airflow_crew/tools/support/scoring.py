...
SCORING_MATRIX = {
    ...
    "dynamic_start_date": {
        "deduction": 35,
        "description": "Dynamic start_date using datetime.now()",
        "category": "Critical",
        "rationale": "Breaks idempotency and causes scheduling issues"
    },
    "no_retries": {
        "deduction": 30,
        "description": "No retry mechanism configured",
        "category": "Critical",
        "rationale": "Critical for task reliability in distributed environments"
    },
    ...
    "dynamic_task_mapping": {
        "deduction": 20,
        "description": "Dynamic task mapping at runtime",
        "category": "Major",
        "rationale": "Can cause DAG parsing issues and scheduler overhead"
    },

    # Minor Issues (5-10% deduction)
    "no_documentation": {
        "deduction": 10,
        "description": "Missing or insufficient DAG documentation",
        "category": "Minor",
        "rationale": "Impacts maintainability and team collaboration"
    },
    "no_tags": {
        "deduction": 5,
        "description": "No tags defined for DAG",
        "category": "Minor",
        "rationale": "Makes DAG organization and filtering difficult"
    },
    "no_sla": {
        "deduction": 5,
        "description": "No SLA defined for critical tasks",
        "category": "Minor",
        "rationale": "Missing SLA monitoring for important tasks"
    }
}
...
def calculate_task_prognosis(task) -> Dict[str, Any]:
    ...
    if not task.retries:
        ...
        issues.append({
            "type": "no_retries",
            "message": "Task has no retry mechanism configured",
    ...
    if not task.execution_timeout:
        ...
        issues.append({
            "type": "no_timeout",
            "message": "Task has no execution timeout set",
            "task_id": task.task_id
        })
    ...
    if task.depends_on_past:
        ...
        issues.append({
            ...
            "message": "Task uses depends_on_past which can cause scheduling issues",
            "task_id": task.task_id
        })
    ...
    if not task.queue:
        ...
        issues.append({
            "type": "no_queue",
            "message": "Task has no specific queue assigned",
            "task_id": task.task_id
        })
...
def calculate_dag_prognosis(dag) -> Dict[str, Any]:
    ...
    if not dag.start_date:
        ...
        issues.append({
            "type": "no_start_date",
            "message": "DAG has no start_date configured"
        })
    ...
    if not dag.doc_md and not dag.description:
        ...
        issues.append({
            ...
            "message": "DAG lacks documentation"
        })
    ...
    if len(dag.tasks) > 50:
        ...
        issues.append({
            ...
            "message": f"DAG has {len(dag.tasks)} tasks, consider breaking it down"
        })
